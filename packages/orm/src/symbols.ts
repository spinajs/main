 ...
 * sits in a require cycle with `orm.ts`, reaching it through `migration-service.js`, `driver.js`
 * and onwards, while `migration-environment.ts` ( which also needs this regexp ) is itself
 * required BY `orm.ts`. Importing it from the runner would close that cycle one hop earlier.
 */
