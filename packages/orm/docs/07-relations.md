...
| `attach(obj \| null)` | Point at a model and write the owner's foreign key to match (the target's join-column value, or NULL). No database access; the owner's `changes()` then reports the key. |
| `detach()` | `attach(null)`. |
