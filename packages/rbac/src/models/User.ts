...
export namespace Commands {
  ...
  export async function unban(identifier: number | string): Promise<User> {
    return _chain(
      _user(identifier),
      async (u: User) => {
  ...
  export async function updateUser(identifier: number | string, data: Partial<User>): Promise<User> {
    return _chain(_user(identifier), _update(data), (u: User) => _ev(new UserChanged(u.Uuid)));
  }

  export async function passwordChangeRequest(identifier: number | string): Promise<User> {
    return _chain(
      _user(identifier),
      async (u: User) => {
  ...
  export async function changePassword(identifier: number | string, password: string): Promise<User> {
    const sPassword = await _service<PasswordProvider>('rbac.password');
  ...
  export async function auth(identifier: string, password: string): Promise<User> {
