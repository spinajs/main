...
@NewInstance()
@Inject(Container)
export class SqlDropViewQueryCompiler extends DropViewCompiler {
  constructor(protected container: Container, protected builder: DropViewQueryBuilder) {
    super();
  }

  public compile(): ICompilerOutput {
    const exists = this.builder.Exists ? ' IF EXISTS' : '';
    const exprr = `DROP VIEW${exists} ${this.container.resolve(TableAliasCompiler).compile(this.builder)}`;
...
export class SqlAlterColumnQueryCompiler extends SqlColumnQueryCompiler {
  ...
  public compile(): ICompilerOutput {
    ...
    if (builder.AlterType === ColumnAlterationType.Rename) {
      ...
      return {
        bindings: [],
        expression: `RENAME COLUMN ${escapeIdentifier(bld.OldName)} TO ${escapeIdentifier(bld.Name)}`,
      };
    }
    ...
    if (builder.AlterType === ColumnAlterationType.Add) {
      return {
        ...
        expression: `ADD ${cDefinition.expression} ${builder.AfterColumn ? `AFTER ${escapeIdentifier(builder.AfterColumn)}` : ''}`,
      };
    }
...
/**
 * Compiler for raw SQL schema queries.
 * Allows executing arbitrary SQL statements within schema context.
 */
@NewInstance()
@Inject(Container)
export class SqlRawSchemaQueryCompiler extends RawSchemaQueryCompiler {
  constructor(protected container: IContainer, protected builder: RawSchemaQueryBuilder) {
    super();
  }
