...
export class MigrateStatusCommand extends CliCommand {
  public async execute(): Promise<void> {
    ...
    for (const e of entries) {
      ...
      // being skimmed in a wall of `applied`.
      const marker = e.failed ? '!!' : '  ';
    ...
    // that were rolled back. The STATE column tells those two apart, so the summary has to as
    // well - otherwise the report says "0 pending" nowhere and "N pending" against a list in
    // which no line reads `pending`.
    const rolledBack = pending.filter((e) => e.rolledBack);
    const neverRun = pending.filter((e) => !e.rolledBack);

    console.log('');
    console.log(`${entries.length} migration(s): ${entries.filter((e) => e.applied).length} applied, ${neverRun.length} pending, ${rolledBack.length} rolled back, ${failed.length} failed, ${entries.filter((e) => e.checksumMismatch).length} with a checksum mismatch`);
  ...
  protected state(e: IMigrationStatusEntry): string {
    return e.failed ? 'FAILED' : e.applied ? 'applied' : e.rolledBack ? 'rolled-back' : 'pending';
  }
